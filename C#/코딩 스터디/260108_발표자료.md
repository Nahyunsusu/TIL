...
public class Player
{
    public void UseSkill(string skillName)
    {
        ...
        {
            // 파이어볼의 복잡한 물리 계산, 파티클 생성, 마나 소모 로직이 여기 다 들어감
            Console.WriteLine("마나 10 소모, 화염구 생성, 포물선 궤적 계산...");
        }
        else if (skillName == "Heal")
        {
            // 힐의 복잡한 회복 수치 계산, 빛 이펙트 로직이 여기 다 들어감
            Console.WriteLine("마나 20 소모, 체력 회복량 계산, 빛 파티클 재생...");
        }
        // 새로운 스킬이 추가될 때마다 Player 클래스를 계속 수정해야 함 (매우 위험!)
    }
}
```
```csharp
// 추상화: 스킬이라면 마땅히 '실행'되어야 한다는 규칙만 정의
public interface ISkill
{
    void Execute();
}

// 구체적인 구현: 각 스킬이 내부적으로 어떻게 도는지 숨김 (캡슐화)
public class Fireball : ISkill
{
    public void Execute() 
    {
        // 복잡한 엔진 로직은 이 안에 숨겨져 있음
        Console.WriteLine("화염구를 발사합니다!");
    }
}
...
public class Player
{
    // 플레이어는 ISkill이라는 '추상적인 규격'만 알고 있음
    public void UseSkill(ISkill skill)
    {
        skill.Execute(); // 어떤 스킬인지 몰라도 '버튼'만 누르면 됨
    }
}
```